...
function guardarConfiguracion() {
    ...
    // Crear FormData para compatibilidad con save-content.php
    const formData = new FormData();
    formData.append('data', JSON.stringify(config));

    fetch('save-config-tech.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
    ...
    .then(data => {
        ...
        if (data.ok || data.success) {
            ...
            console.log('✅ Secciones actualizadas:', data.updated_sections);
        } else {
            mostrarNotificacion('❌ Error: ' + (data.msg || data.error || 'Error desconocido'), 'error');
        }
    })
    .catch(error => {
...
function validarFormulario() {
    ...
    const fontMain = document.getElementById('fontMain').value.trim();
    
    if (!brandName) {
        mostrarNotificacion('⚠️ El nombre de la marca es obligatorio', 'error');
        return false;
    }
    
    if (!fontMain) {
        mostrarNotificacion('⚠️ La fuente principal es obligatoria', 'error');
        return false;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🔧 Configuración técnica lista');
    mostrarNotificacion('📝 Configuración técnica cargada', 'success');
});
